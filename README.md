...
项目已从比赛演示架构转向内测版架构。代码在一个仓库中同时保留完整前端和后端，运行时是两个独立服务：使用者只打开 `5173` 上的图形前端，页面再调用 `18768` 上的后端 API。当前前端已恢复原有吉祥物、语音优先首页、照片上传、记录时间线、看病资料和设置页；同时完成本地加密主数据、所有者访问保护、无状态 AI 边界、加密备份下载/预览/恢复、私有 TOS 短时授权接线、时间线筛选、修订历史、选中删除、就诊交接材料和内测问题记录。
...
2026-09-17 当前工作树验证结果：Python `765 passed`，Node `36 passed`，TypeScript 合同检查通过。真实本机浏览器也已从完整前端 `5173` 完成加密仓库、文字记录、刷新解锁、本机持久恢复和跨端口后端边界检查。这些结果证明当前代码回归，不替代真实手机和正式云环境验收。
