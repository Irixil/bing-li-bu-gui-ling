...
STATIC = (ROOT / "frontend" / "dist").resolve()
SESSION_TOKEN = secrets.token_urlsafe(24)
DB_PATH = os.getenv(
    "DB_PATH", os.getenv("API_DB_PATH", str(ROOT / "runtime" / "records.sqlite3"))
)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def end_headers(self):
        ...
        expected = ALLOWED_ORIGIN or ("http://" + self.headers.get("Host", ""))
        if o and o == expected:
    ...
    def csrf(self):
        if self.command == "GET":
            return True
        if urlparse(self.path).path in ("/api/auth/households", "/api/auth/login"):
            return True
    ...
    def _auth_ctx(self, permission=None, household_id=None):
        ...
        try:
            return (
                STORE.authorize(token, permission, household_id)
                if permission
    ...
    def do_OPTIONS(self):
        ...
        self.send_header(
            ...
            "Content-Type, Idempotency-Key, X-Session-Token, X-Auth-Token",
        )
        self.end_headers()
    ...
    def do_GET(self):
        try:
            ...
            return self.send_json(e.status, {"ok": False, "error": str(e)})
        except Exception:
    ...
    def _do_GET(self):
        ...
        if p == "/api/events":
            ...
            return self.send_json(
                ...
                {
                    "ok": True,
                    "events": STORE.list(ctx["household_id"] if ctx else None),
                },
            )
        if p == "/api/auth/me":
            try:
                return self.send_json(
                    ...
                    {
                        "ok": True,
                        "user": STORE.authenticate(self.headers.get("X-Auth-Token")),
                    },
                )
            except StoreError as e:
                return self.send_json(e.status, {"ok": False, "error": str(e)})
        if p == "/api/auth/members":
            try:
                ctx = STORE.authorize(self.headers.get("X-Auth-Token"), "read")
                return self.send_json(
                    200,
                    {"ok": True, "members": STORE.list_members(ctx["household_id"])},
                )
            except StoreError as e:
                return self.send_json(e.status, {"ok": False, "error": str(e)})
        if p.startswith("/api/events/"):
            z = p.strip("/").split("/")
            rid = z[2] if len(z) > 2 else ""
            try:
                if len(z) == 4 and z[3] == "history":
                    ctx = self._auth_ctx()
                    if self.headers.get("X-Auth-Token") and ctx is None:
                        return self.send_json(
                            401, {"ok": False, "error": "invalid_session"}
                        )
                    if ctx and not STORE.get(rid, ctx["household_id"]):
                        return self.send_json(
                            404, {"ok": False, "error": "event_not_found"}
                        )
                    h, a = STORE.history(rid)
                    return self.send_json(200, {"ok": True, "history": h, "audit": a})
                if len(z) == 3:
                    ctx = self._auth_ctx()
                    if self.headers.get("X-Auth-Token") and ctx is None:
                        return self.send_json(
                    ...
                    e = STORE.get(rid, ctx["household_id"] if ctx else None)
                    return (
                        self.send_json(200, {"ok": True, "event": e})
                        if e
                        else self.send_json(
                            404, {"ok": False, "error": "event_not_found"}
                        )
        ...
        if p.startswith("/api/handoffs/"):
            try:
                ...
                if ctx and handoff.get("household_id") not in (
                    None,
                    ctx["household_id"],
                ):
                    return self.send_json(
                        404, {"ok": False, "error": "handoff_not_found"}
                    )
    ...
    def do_POST(self):
        if not self.csrf():
            return self.send_json(
                403, {"ok": False, "error": "csrf_or_origin_rejected"}
            )
        ...
        try:
            ...
            return self.send_json(400, {"ok": False, "error": "invalid_json_payload"})
        actor = str(b.get("actor_name") or "本地用户")
        try:
            if p == "/api/auth/households":
                result = STORE.create_household(
                    b.get("name"),
                    b.get("display_name"),
                    b.get("role", "owner"),
                    b.get("external_key"),
                ...
                return self.send_json(
                    201, {"ok": True, "household": result, "session": login}
                )
            if p == "/api/auth/login":
                return self.send_json(
                    200,
            ...
            if p == "/api/events":
                for k in ("raw_text", "source_kind", "actor_name"):
                    if not isinstance(b.get(k), str) or not b[k].strip():
                        raise StoreError(k + "_required")
                if len(b["raw_text"]) > 10000:
                    raise StoreError("raw_text_too_long")
                ...
                e, created = STORE.create(
                    b, key, actor, ctx["household_id"] if ctx else "hh_local_default"
                )
                return self.send_json(
                    201 if created else 200,
                    {"ok": True, "created": created, "event": e},
                )
            if p == "/api/handoffs":
                ...
                return self.send_json(
                    ...
                    {
                        "ok": True,
                        "handoff": STORE.handoff(ctx["household_id"] if ctx else None),
                    },
                )
            z = p.strip("/").split("/")
            if len(z) >= 4 and z[:2] == ["api", "events"]:
                ...
                if z[3] == "organize":
                    ...
                    related = [STORE.get(x) for x in e.get("related_record_ids", [])]
                    payload = {**e, "history": related}
                    ...
                    return self.send_json(
                        200, {"event": out, "raw_text_preserved": True, **r}
                    )
                if z[3] == "review":
                    if ctx:
                        STORE.authorize(
                ...
                if z[3] == "revise":
                    ...
                    return self.send_json(
                        201,
                        {
                            "ok": True,
                            "event": STORE.revise(
                                rid, b.get("expected_version"), b, actor
                            ),
                        },
                    )
        except StoreError as e:
...
def serve(host="127.0.0.1", port=None):
    ThreadingHTTPServer(
        (host, int(os.getenv("API_PORT", port or 18768))), Handler
    ).serve_forever()
