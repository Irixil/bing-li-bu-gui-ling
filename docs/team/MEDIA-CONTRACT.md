...
状态：**本地 MVP 已实现；正式可调用形状以 API v0.4 为准，外部设备与生产仍有缺口**。

更新日期：2026-09-14。
当前实现基线：`feaf182`；历史需求基线为 `6e1a7f3`。
...
- 仓库合成 WAV/PNG 已完成一次真实供应商全链路；浏览器真实 WebM 曾识别失败但原件保留。真实浏览器播放、手机暂停续录合并、通用格式、真实患者、生产和临床仍未验证。
