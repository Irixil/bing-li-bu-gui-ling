...
实现基线：当前 `fix/backend-api` 分支的媒体整合提交；依赖 B 的 `backend/recognition.py`。
...
- `backend/media_backend.py`：把文件、SQLite 和识别流程接成默认本地适配器；默认是明确标注的离线 Mock 模式，配置真实 provider 后不会静默回退。
- `backend/server.py`：multipart 创建/分片/完成、媒体列表/详情/原件 Range 读取、202 识别和已保存文字的 `/link` 恢复入口。
- `scripts/backup.py`：SQLite 与被引用原件的一致备份、清单校验、空目录恢复和损坏/路径越界拒绝。
- `docs/API.md`、`contracts/api.ts`、`frontend/README.md`：同步当前实际 HTTP 形状和前端接线规则。

## 当前可依赖的行为

原件只有在文件完整校验、数据库元数据提交后才显示 `save_status=saved`。识别返回 `202` 后轮询详情；机器初稿、危险扫描和 Event 链接分别持久化。识别失败、超长文字、无 Event 媒体仍可查询；交接快照会列出已保存但未关联的媒体附件。`/link` 复用已有成功初稿，不重复调用识别。

保留完整机器文字，不恢复 60 秒限制，不自动诊断或改药。默认 Mock 必须在页面显示“离线 Mock 演示模式”；未命中危险词不代表医学正常。
...
媒体专项还覆盖原件存储、状态机、识别编排、默认后端、HTTP 适配和备份恢复。证据规则与未验证项见 `docs/evidence/media-a/README.md`。
