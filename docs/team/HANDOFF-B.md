...
状态：识别模块已完成并由 A 侧媒体后端接入；真实 ASR/OCR 成功证据尚未验证。媒体上传、数据库状态、Event 关联和 HTTP 合同已在 `fix/backend-api@89a2ee2` 组装并通过本地自动化回归。
...
媒体端点的当前字段和状态以正式 `docs/API.md`、`contracts/api.ts` 和 `frontend/README.md` 为准；`docs/team/MEDIA-CONTRACT.md` 保留产品决策与验收边界，不能覆盖正式运行合同。
