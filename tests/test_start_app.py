...
def test_check_config_fails_when_owner_access_is_not_configured(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(start_app.app_access, "configured", lambda: False)
...
def test_check_config_passes_only_when_models_and_owner_access_are_ready(tmp_path, monkeypatch):
    monkeypatch.setattr(start_app, "prepare_app_environment", lambda _root: ({"API_PORT": "18768"}, False))
    monkeypatch.setattr(start_app, "check_demo_configuration", lambda _env: {"模型": []})
    monkeypatch.setattr(start_app.app_access, "configured", lambda: True)
